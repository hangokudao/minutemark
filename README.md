...
실제 회의 음성을 글로 옮기고, 회의에서 확정된 결정과 할 일을 근거가 된 실제 말과 함께
보여주는 AI 회의 노트입니다.
...
V2에는 Google 회원가입·로그인, 사용자별 회의 저장·재열람, 회의 삭제,
재인증 기반 계정 탈퇴와 개인정보처리방침까지 추가했습니다. PR #5를 `main`에
병합한 commit `48b76d2…`의 `minutemark-00012-luh`를 공개 트래픽 100%로
배포했습니다. 협업, 댓글, 실시간 회의 봇은 계속 제외했습니다.
...
flowchart LR
    ...
    B --> C["faster-whisper small<br/>CPU INT8 음성 인식"]
    C --> D["음성 인식 결과와 구간 ID만<br/>A6API GPT-5.6 Luna로 전송"]
    D --> E["결정·할 일 JSON"]
    E --> F["서버 구조·근거 검증"]
...
음성 원본은 A6API에 보내지 않습니다. 서버에서 Whisper로 글로 옮긴 내용과
구간 ID만 전송합니다.
...
- 결과의 `근거 듣기`에서 재생 위치 이동과 해당 기록 강조
- Firebase Authentication 기반 Google 회원가입·로그인·로그아웃
- Firebase UID 소유권으로 분리된 회의 목록·상세와 5분 signed audio URL
- 성공한 분석만 Firestore·비공개 Storage에 저장하고 실패하면 남은 오디오를 삭제
- 같은 요청을 반복해도 중복되지 않는 분석, 회의 삭제, 최근 Google 재인증 후 콘텐츠부터 지우는 계정 탈퇴
...
아래 표는 2026-07-31 Windows Chrome에서 공개 V1을 직접 실행한 기준선입니다.
현재 공개 서비스는 같은 Luna 분석 흐름에 회원 저장·삭제를 더한 V2입니다.

| 입력 | 전체 처리 | 결과 | 근거 이동 | 예상 A6 비용 |
| --- | ---: | --- | --- | ---: |
| 법안 통과 후속 작업 | 39.75초 | 음성 기록 7구간, 할 일 1개 | S5·S6 → 약 23.72초 | $0.00008890 |
...
- `response_format=json_schema`의 HTTP 400: 일반 JSON 요청으로 한 번 대체 요청
- 대체 요청의 응답: 서버에서 필수 필드와 타입을 다시 검증
- 모델이 `"S5, S6"` 문자열을 반환한 사례: 구간 ID를 정규화한 뒤 실재 여부 재검증
- 검증 실패: 사용자 결과로 내보내지 않고 안전한 오류로 종료

공개 환경에서도 실제 400이 발생했으며, 대체 요청 후 HTTP 200과 근거 검증 통과를
확인했습니다.

## Docker와 배포

- 애플리케이션: FastAPI + Vanilla JavaScript
- 음성 인식: `faster-whisper 1.2.1`, `small`, CPU INT8
- 결과 정리: A6API `gpt-5.6-luna`
- 패키징: Docker
...
GitHub `main`이 배포 기준입니다. `main`에 병합하면 Cloud Build가
...
- A6API가 음성을 글로 옮긴 내용을 얼마나 보관하는지, 학습에 사용하는지, 어느 나라에서 처리하는지,
  다른 업체에 맡기는 조건은 확인되지 않았습니다. 포트폴리오에서는 실제 업무·기밀·민감정보 업로드를 금지하고
  음성 인식 결과 전송을
  고지하며, 정식 서비스로 전환할 때 제공자 조건을 다시 확인합니다.
- Cloud Run의 로컬 SQLite 예산 장부는 인스턴스 재시작 시 초기화될 수 있습니다.
- 공개 데모는 한 번에 한 분석만 처리합니다.
- A6 스마트 라우터 판매자에 따라 결과를 항목별로 받는 기능이 달라 서버 대체 경로와 검증을
  사용합니다.

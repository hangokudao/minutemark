...
> [`V2.1 버전업 실행 기록`](./V2_1_RELEASE_PLAN.md)을 최신 기준 문서로 사용한다.
...
대체 경로는 당시 개인 Google 계정 선택과 동의, 입력 경계, 모바일 메뉴,
...
| 계정 탈퇴 | 후보 `/account` → `/samples`; 1440×900 | `탈퇴` 입력, Google 재인증, MinuteMark B 계정 삭제 | 공개 샘플·로그아웃 UI 복귀, 삭제 회의 주소 비노출 | `DELETE /api/account` 204; B Firestore 문서·Storage 객체·삭제되지 않고 남은 객체 0, Firebase Auth 사용자 부재 | 사용자 첨부·Chrome inline 캡처, 저장 경로 없음 | PASS |
...
| F. 상세·오디오·근거 듣기 | 로컬 `/meetings/{id}` 예정 | 1440×900 | 상세·재생·근거 이동 예정 | 실제 재생 위치 이동과 음성 기록 강조 | 미수행 | signed URL 미발급 | 없음 | 미검증 |
...
| I. 로그아웃 뒤 비노출 | `http://localhost:8000/samples`, `/meetings` | 1440×900 | 실제 로그아웃 뒤 개인 경로 직접 진입 | 개인 DOM 제거, 인증 화면으로 전환 | 계정 표기가 `Google로 시작하기`로 바뀌고 빈 목록 DOM은 숨겨졌으며 `/meetings`는 `/auth?next=%2Fmeetings`로 전환됨. 실제 저장 회의·음성 기록·signed URL이 있던 세션의 DOM 제거는 확인하지 못함 | 공개 API 200, 개인 요청은 로그인 화면에서 차단 | `C:\Users\myhan\AppData\Local\Temp\minutemark-v2-windows-signed-out-private-route.png` | 미검증 |
...
| L. 계정 탈퇴 | 후보 `/account` 예정 | 1440×900 | `탈퇴` 입력·Google 재인증 예정 | 회의·남은 객체·Auth 사용자 모두 0 | 코드·mock 회귀만 통과 | 실제 Auth·저장소 변화 미수행 | 없음 | 미검증 |
...
[V2.1 버전업 실행 기록](./V2_1_RELEASE_PLAN.md)을 최신 기준으로 따른다.
...
- A6API 공개 사이트에서 공급자 보관 기간·학습 사용·처리 국가·재위탁 조건을
  ...
  출시 차단 판단이다. 최신 포트폴리오 후보는 민감정보 업로드 금지와 음성 인식 결과 전송을

...
`Whisper 음성 인식 → A6API 결과 정리 → 근거 구간 검증 → 근거 듣기` 흐름 바깥에
...
FastAPI가 검증한 사용자 요청만 처리한다.

설계 당시 판정은 `CONDITIONAL GO`였다. V2.1 포트폴리오 범위에서는 아래 1번과
3번을 확인하고, 실제 업무·기밀·민감정보 업로드 금지와 A6API 음성 인식 결과 전송을
명확히 고지한다. 2번은 정식 서비스 전환 전 출시 차단 조건으로 유지한다.
...
결정을 뒤집는 조건은 하나다. A6API의 데이터 처리 조건이 회원 회의의 음성 인식 결과를
보내기에 적절하지 않다면 회원 기능 공개를 보류하고 결과를 항목별로 정리하는 공급자를 먼저
교체한다.
...
- 협업, 팀, 공유 링크, 댓글
- 요금제, 결제, 구독
- 실시간 회의 봇, 실시간 음성 인식, 백그라운드 작업 큐
- 검색, 태그, 폴더, 즐겨찾기
- 화자 분리, 음성 기록 수정, 할 일 완료 처리
- 캘린더 연동, 내보내기, 재분석 버전
- 관리자 대시보드, 다중 인증 공급자
...
- `INTERACTION`: `근거 듣기`는 현재처럼 재생 위치 이동·재생·관련 음성 기록 강조를 한 번에
...
3. 업로드 영역 바로 아래에 다음 사실을 짧게 알린다.
   `분석 완료 시 원본 오디오와 음성 인식 결과·분석 결과를 내 회의 기록에 저장합니다.`
...
현재 서버는 단계별 진행 이벤트를 보내지 않으므로 `음성 인식 70%`, `저장 중 90%` 같은
...
`새 회의` 버튼만 보여준다. 음성 기록 일부, 토큰, 비용, 모델은 상세에서만 표시한다.
...
ID token, 이메일, 음성 인식 결과를 로그에 남기지 않고, HTML에는 `Referrer-Policy:
...
| AI 결과 | 음성 인식 결과·시각·segment ID, 결정, 할 일, 담당자·기한 | 회의 기록 제공 |
| 운영 | route, 요청 시각, 상태, latency, 오류 코드, 사용량·비용 | 장애 대응·비용 보호 |

Firebase Auth가 제공할 수 있는 이름·사진은 앱 DB에 복제하지 않는다. 음성 인식 결과,
...
- 회원 업로드 실제 1회, 새로고침 후 다시 열기, 회의 삭제 확인
...
2. AI는 음성 기록을 바탕으로 결과를 정리하고 일반 코드는 근거 구간과 사용자 소유권을 보장한다.
...
- 음성 인식 결과·AI 문자열·제목을 escaping 없이 `innerHTML`에 삽입함
- ID token, signed URL, 이메일, 음성 인식 결과를 로그에 남김
...
출시 증거 수집이었다. 배포 후보의 공개 화면, 계정 A·B 로그인, 실제 분석·저장→
새로고침 후 다시 열기→모바일 회의 삭제, 다른 사용자 직접 주소 차단과 계정 탈퇴 뒤
Firestore·Storage·Firebase Auth 부재까지 확인했다. PR #5 병합, main SHA의 0%
후보 확인, 공개 V2 전환과 데스크톱·모바일 스모크도 완료했다. A6API의 상세 데이터
처리 조건은 포트폴리오 경고와 음성 인식 결과 전송 고지로 현재 한계를 명시하며, 정식 서비스
전환 전 별도 확인한다.

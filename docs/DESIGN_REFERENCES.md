...
조사일: 2026-08-02
대상 흐름: `공개 샘플/음성 업로드 → 시간 정보가 붙은 음성 기록 → 결정·할 일 → 근거가 된 말로 이동`
...
1. **Notion AI Meeting Notes** — 요약의 인용에 마우스를 올리면 음성 기록 일부를 미리 보고, 클릭하면 해당 기록으로 이동한다. MinuteMark의 `근거 듣기`를 가장 직접적으로 다듬을 수 있는 참고 사례다.
2. **Microsoft Teams Recap** — 재생 타임라인의 마커, 음성 기록 패널, AI 요약·후속 작업을 하나의 recap 경험으로 묶는다. 결과 화면의 큰 정보 구조에 가장 유용하다.
3. **Otter.ai Conversation Page** — `Summary / Transcript` 분리와 각 할 일의 `View in transcript` 연결이 MinuteMark의 결과 모델과 거의 같다.

**조합 권고:** Teams처럼 상단에 고정 재생기와 시간축을 두고, Otter처럼 본문을 `회의 결과 / 음성 기록` 두 영역으로 명확히 나눈다. 각 결정·할 일에는 Notion식 작은 근거 칩을 붙여 마우스를 올렸을 때 인용문·시간을 미리 보여주고, 클릭하면 재생 위치 이동과 해당 기록 강조를 함께 실행한다. 첫 화면과 처리 중 화면은 이 결과 구조로 들어가기 위한 짧은 진입 단계로만 유지한다.
...
- **확인된 유사 기능:** 녹화 타임라인 마커를 눌러 해당 시점으로 이동할 수 있고, 음성 기록을 바탕으로 한 AI 요약에서 노트와 후속 작업을 제공한다.
...
- **확인된 유사 기능:** 녹화를 스마트 챕터로 나누고, 하이라이트·요약·다음 단계를 제공한다. 챕터를 클릭하면 그 시점부터 재생되며 음성 기록 패널을 함께 관리할 수 있다.
- **참고할 패턴:** 긴 음성 기록을 몇 개의 `시간이 붙은 챕터`로 압축하고, 플레이어 옆에 현재 구간의 기록을 두는 구성.
- **주의:** 편집·분석·코칭 지표까지 노출하면 결정과 할 일의 우선순위가 약해진다. 챕터는 음성 기록이 긴 경우에만 보조 탐색으로 쓴다.
...
- **주의:** 여러 화면으로 분리된 구조를 그대로 따르면 근거 연결이 숨는다. MinuteMark에서는 결과 카드와 음성 기록을 같은 화면에 유지한다.
...
- **확인된 유사 기능:** 실시간 대화와 huddle thread 메시지를 바탕으로 핵심 요점과 할 일을 Canvas에 만들고, 음성 기록을 그 Canvas에 함께 넣는다.
...
- **확인된 유사 기능:** 실시간 녹음뿐 아니라 기존 오디오 업로드도 음성을 글로 옮겨 요약하며, 핵심 요점과 할 일을 만든다. 요약의 인용에 마우스를 올리면 기록 일부를 보고 클릭하면 해당 기록으로 이동한다.
- **참고할 패턴:** 결과 문장 끝의 작고 일관된 `근거 인용 칩`; hover 미리보기 후 클릭 이동이라는 2단계 확인 경험.
- **주의:** 문서 편집기·템플릿·데이터베이스 기능까지 닮게 만들면 제품 정체성이 흐려진다. MinuteMark의 클릭은 기록 이동뿐 아니라 재생 위치 이동까지 분명히 보여야 한다.
...
- **확인된 유사 기능:** `Summary / Transcript` 탭에서 자동 요약·할 일·개요와 음성 기록을 제공하며, AI가 만든 할 일의 `View in transcript` 아이콘으로 생성 근거가 된 기록 위치를 연다.
- **참고할 패턴:** 결과와 원문을 나누되 한 번에 전환 가능한 2탭 구조; 각 할 일 우측의 작고 반복 가능한 `음성 기록에서 보기` 액션.
...
- **확인된 유사 기능:** 한 회의 화면에 AI 요약·전체 음성 기록·녹화·할 일을 모으고, `왼쪽 요약 / 오른쪽 음성 기록` 2패널을 제공한다. 재생은 기록과 동기화되고 기록을 선택해 Soundbite를 만들 수 있다.
- **참고할 패턴:** 데스크톱 결과 화면의 명확한 2열 비대칭 레이아웃; 현재 재생 중인 기록 문장을 패널 안에서 강조한다.
...
- **확인된 유사 기능:** 음성 기록·사용자 메모·일정 정보를 합쳐 정리된 노트를 만들고, 각 노트 옆 돋보기로 그 내용이 나온 기록 또는 원문 메모를 확인할 수 있다.
...
- **위계:** 회의 제목·길이 → 결정 → 할 일 → 음성 기록 순서로 읽히게 하고, 처리 시간·비용은 보조 정보로 낮춘다.
- **근거:** 모든 결정·할 일에 같은 근거 칩을 사용한다. `S5·S6` 같은 내부 ID보다 `00:24 · 실제 말 2개`처럼 사람이 이해하는 표기를 우선한다.
- **이동 피드백:** 근거를 클릭하면 플레이어 시간 이동, 해당 기록 자동 스크롤, 2~3초 강조를 한 동작으로 묶는다.
- **레이아웃:** 넓은 화면은 결과와 음성 기록을 2열로, 좁은 화면은 `결과 / 음성 기록` 탭으로 바꾼다. 플레이어는 두 상태 모두 화면 안에 남긴다.
...
**근거:** MinuteMark의 차별점은 일반적인 AI 요약이 아니라 결정·할 일이 실제 말이 나온 구간으로 되돌아가 확인할 수 있다는 점이다.
**확신도:** 높음.
**다음 행동:** 구현 전, 이 조합으로 데스크톱 결과 화면 와이어프레임 1개만 만든다.
**종료 기준:** 결정·할 일 하나를 보고 근거가 된 실제 말을 재생하기까지 한 번의 클릭이면 되는 구조가 확인되면 참고 사례 탐색을 종료한다.
